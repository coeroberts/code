--- Block markets only, one month data


-- blocks puchased and ad spend on blocks by professional, makret and ad type in previous month
WITH 
...
(
		...
		left join dm.ad_mkt_dim amd on amd.ad_mkt_id = OLAMF.ad_market_id
...
-- 		, ard.ad_regn_id as ad_regn_id
	, AMD.AD_MKT_REGN_NAME AS Ad_Region
	, AMD.AD_MKT_STATE_NAME AS Ad_State
	--       ,MAX(AMD.AD_MKT_CNTY_NAME) AS AD_MKT_CNTY_NAME
	, AMD.AD_MKT_SPECLTY_NAME AS Practice_Area
...
left join dm.ad_mkt_dim amd on amd.ad_mkt_key = cpp .ad_mkt_key
...
-- left join dm.ad_regn_dim ard on ard.ad_regn_key = amd.ad_regn_key
