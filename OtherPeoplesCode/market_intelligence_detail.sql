...
(
              ...
              (
                             select p.PROFNL_ID
                                           , lower(p.PROFNL_STATE_NAME_1) as state
                                           , lower(p.PROFNL_CNTY_NAME_1) as county
                                           , case when p.PROFNL_CLAIM_DATETM is null then 'not-claimed' else 'claimed' end as is_claim
                                           , case when p.PROFNL_EMAIL_ADDR is null then 'no-email' else 'has-email' end as has_email
