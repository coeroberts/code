...
INSERT INTO tmp_data_dm.coe_churn_by_cohort_raw
  SELECT
    ...
    ,MIN(m2.month_begin_date)  AS                         tenure_start_date
    ,MAX(m2.month_end_date)  AS                           tenure_end_date
...
INSERT INTO tmp_data_dm.coe_churn_by_cohort_raw
  SELECT
    ...
    ,CAST(FLOOR(AVG(chn.tenure_mid_num)) AS INT)  AS      tenure_mid_num
    ,MIN(m2.year_begin_date)  AS                          tenure_start_date  -- This is a hack to make 2013 work.
    ,MAX(m2.month_end_date)  AS                           tenure_end_date
    ,MAX(NULL) AS                                         tenure_year_month
    ,SUM(chn.data_months) AS                              data_months
    ,CAST(SUM(chn.new_customers_sum) AS INT) AS           new_customers_sum
...
FROM
  ...
  SELECT
    ...
    ,raw.tenure_end_date
     -- Tenure end date ends up more complicated, because you have to
     -- add (the length of the cohort -1) to the last month in the cohort.
     -- Formula ends up being add (cohort_length*2)-1 to tenure start month
     -- and take the day before that.
    ,from_unixtime(unix_timestamp(
       CAST(raw.tenure_start_date AS TIMESTAMP) + INTERVAL cohort_length*2-1 MONTH - INTERVAL 1 DAY)
