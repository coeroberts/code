...
SELECT 
  ...
  ,mrr.year_month
  ,mrr.chain_number
  ,mrr.tenure_month_chain
  ,mrr.tenure_month_lifetime
  ,mrr.active_months_to_date
  ,mrr.start_month_chain
  ,mrr.start_month_lifetime
  ,mrr.first_payment_month
  ,mrr.has_ads
  ,mrr.has_payment
  ,mrr.customer_exists_prior_month
  ,mrr.max_bill_date_in_month
  ,mrr.revenue_current_advertisement
  ,mrr.revenue_current_avvopro
  ,mrr.revenue_current_ignite
  ,mrr.revenue_current_website
  ,mrr.revenue_current_adplacement
  ,mrr.revenue_current_misc
  ,mrr.revenue_current_other_sub
  ,mrr.revenue_current_other
  ,mrr.revenue_current_total
  ,mrr.mrr_current_advertisement
  ,mrr.mrr_current_avvopro
  ,mrr.mrr_current_ignite
  ,mrr.mrr_current_website
  ,mrr.mrr_current_adplacement
  ,mrr.mrr_current_other_sub
  ,mrr.mrr_current_total
  ,mrr.tenure_month_prev
  ,mrr.has_ads_prior
  ,mrr.has_payment_prior
  ,mrr.revenue_prior_advertisement
  ,mrr.revenue_prior_avvopro
  ,mrr.revenue_prior_ignite
  ,mrr.revenue_prior_website
  ,mrr.revenue_prior_adplacement
  ,mrr.revenue_prior_misc
  ,mrr.revenue_prior_other_sub
  ,mrr.revenue_prior_other
  ,mrr.revenue_prior_total
  ,mrr.mrr_prior_advertisement
  ,mrr.mrr_prior_avvopro
  ,mrr.mrr_prior_ignite
  ...
  ,mrr.mrr_retained
  ,mrr.mrr_returned
  ,mrr.mrr_retained_subset_flat_customers
  ,mrr.mrr_retained_subset_delta_customers
  ,mrr.has_had_payment_lifetime
  ,mrr.mrr_customer_exception
FROM tmp_data_dm.coe_jrr_backfill_customer_mrr mrr
WHERE mrr.year_month = 201608
;
...
SELECT
  ...
  ,CASE WHEN new.customer_id IS NULL THEN 'See exception field'
        WHEN new.mrr_customer_category = off.mrr_customer_category THEN 'OK'
        ...
        ELSE 'Category mismatch' END AS chk_category
  ...
  ,new.has_payment AS                          new_has_payment
  ...
  ,new.has_payment_prior AS                    new_has_payment_prior
...
Oooh!  Put filter ligic into active_months as a flag, and then it''s
...
revenue_plug
  ...
  revenue
from cust_months
filter: NOT (has_ads_during_month = 'Y' or potential_mrr_total > 0) AND revenue <> 0
  (which is is_active_during_month = 'N' AND revenue <> 0)
