...
SELECT
  ...
  ,LAG(retained_customers_sum) OVER(PARTITION BY cust_type, cohort_num 
                                        ORDER BY tenure_num) AS prior_month_retained_customers_sum
  ,LAG(retained_customers_avg) OVER(PARTITION BY cust_type, cohort_num 
                                        ORDER BY tenure_num) AS prior_month_retained_customers_avg
  ,LAG(retained_customers_sum) OVER(PARTITION BY cust_type, cohort_num 
                                        ORDER BY tenure_num)
   - retained_customers_sum AS                                  churned_customers_sum
  ,LAG(retained_customers_avg) OVER(PARTITION BY cust_type, cohort_num 
                                        ORDER BY tenure_num)
   - retained_customers_avg AS                                  churned_customers_avg
  ,new_mrr_sum
  ,retained_mrr_sum
  ,retained_mrr_avg
  ,LAG(retained_mrr_sum)       OVER(PARTITION BY cust_type, cohort_num 
                                        ORDER BY tenure_num) AS prior_month_retained_mrr_sum
  ,LAG(retained_mrr_avg)       OVER(PARTITION BY cust_type, cohort_num 
                                        ORDER BY tenure_num) AS prior_month_retained_mrr_avg
  ,LAG(retained_mrr_sum)       OVER(PARTITION BY cust_type, cohort_num 
                                        ORDER BY tenure_num)
   - retained_mrr_sum AS                                        churned_mrr_sum
  ,LAG(retained_mrr_avg)       OVER(PARTITION BY cust_type, cohort_num 
                                        ORDER BY tenure_num)
   - retained_mrr_avg AS                                        churned_mrr_avg
FROM
  (
  SELECT
...
INSERT INTO tmp_data_dm.coe_churn_by_cohort_raw
  SELECT
    ...
    ,CAST(SUM(chn.prior_month_retained_customers_sum) AS INT) AS  prior_month_retained_customers_sum
    ,CAST(AVG(chn.prior_month_retained_customers_avg) AS INT) AS  prior_month_retained_customers_avg
    ...
    ,CAST(SUM(chn.prior_month_retained_mrr_sum) AS DECIMAL(20,2)) AS  prior_month_retained_mrr_sum
    ,CAST(AVG(chn.prior_month_retained_mrr_avg) AS DECIMAL(20,2)) AS  prior_month_retained_mrr_avg
...
SELECT
  ...
  ,retained_customers_sum
  ,retained_customers_avg
  ,prior_month_retained_customers_sum
  ,prior_month_retained_customers_avg
  ...
  ,retained_mrr_sum
  ,retained_mrr_avg
  ,prior_month_retained_mrr_sum
  ,prior_month_retained_mrr_avg
    ,churned_mrr_sum
    ,churned_mrr_avg
FROM
  (
  ...
  SELECT
    ...
    ,raw.data_months
    ,raw.new_customers_sum / (raw.data_months / raw.cohort_length) AS       new_customers_sum
    ,raw.retained_customers_sum / (raw.data_months / raw.cohort_length) AS  retained_customers_sum
    ,raw.retained_customers_avg
    ,raw.new_mrr_sum / (raw.data_months / raw.cohort_length) AS             new_mrr_sum
    ,raw.retained_mrr_sum / (raw.data_months / raw.cohort_length) AS        retained_mrr_sum
