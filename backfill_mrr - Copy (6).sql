...
SELECT
  ...
  ,CASE WHEN uns.product_line_id IN (2, 7, 18) AND  -- LOOK check all max_expired_date handling.
...
FROM
  (
      SELECT
        ...
        -- ,GREATEST(CASE WHEN olaf.order_line_cancelled_date = '-1' THEN mrr.expired_date ELSE olaf.order_line_cancelled_date END
        --          ,mrr.expired_date) AS max_expired_date_g
  ...
  UNION ALL
      SELECT
        ...
        --       ELSE from_unixtime(unix_timestamp(sub.expire_datetime), 'yyyy-MM-dd')
        --  END AS max_expired_date
        -- ,GREATEST(CASE WHEN olaf.order_line_cancelled_date = '-1' 
        --                  THEN NULL ELSE olaf.order_line_cancelled_date END
        --          ,CASE WHEN TRUNC(sub.expire_datetime, 'DAY') = '1900-01-01'  
        --                  THEN NULL ELSE from_unixtime(unix_timestamp(sub.expire_datetime), 'yyyy-MM-dd') END) AS max_expired_date_g
...
SELECT
   ...
   ,adj.has_non_ads_eom
   ,CASE WHEN adj.tenure_month_lifetime = 1 THEN 'N' ELSE 'Y' END AS customer_exists_prior_month
   ,adj.mrr_method
...
WHERE chn.year_month BETWEEN 201304 AND 201611
)
...
WHERE mth.is_active_during_month = 'N'
  AND mth.year_month BETWEEN 201304 AND 201611 ;
