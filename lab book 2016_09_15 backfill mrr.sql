...
-- WHERE olaf.order_line_begin_date >= '2010-01-01'
...
This was in a different file:
Get all billed months.
    Only counts if MRR > 0 OR has_ads.
    Get whether any successful payment in month.
    Add a has_ads flag.

Group billed months into chains.
    Categorize into ACQUIRED vs. RETURNED.
    Maybe could do all MRR categories here?  Nope.
    ACQUIRED, RETURNED, CHURNED, CONTINUED.

Can identify from chains:
    ACQUIRED
    RETURNED
    CHURNED
Requires revenue:
    ...
    RETAINED
    NO ACTIVITY
Need to add these at the end:
    NOT BILLED

Jake:
-- Can''t change definition based on successful payment or not.  Can provide a flag.  Right.
-- Old history (< 201410) can''t look at active after EoM as a criteria.  OK.
What counts as total revenue and MRR?  Include 'Other'?  In backfill,
  if there is a sub_id.
  What if subscription or not?  Yep.
If customer had MRR > 0 last month and 0 this month, do we want to call them
DOWNSIZED?  Then in subsequent months they would count as NO ACTIVITY.
Ah shoot, that would also affect acquired.
Downsized and Upsold.  
...
Differences:
- only count mrr for unexpired subs
- feb 2016 leap year

Should revenue include misc payments? separate.


QUESTIONS:
- Should I change granularity of billed_months table to 1 per (customer_id, year_month)?
  Or have another table that changes to that granularity before building chains?
  Probably.
  Maybe do not have to carry through all MRR categories into chains,
  just wrap back to them after I have done the categorization?

    coe_jrr_customer_billed_months
        customer_id
        product_line_id
        ad_type
        year_month
    ...
    coe_jrr_customer_start
        customer_id
        start_month
        start_month_begin_date

    coe_jrr_customer_start_month
        customer_id
        start_month
        start_month_begin_date
        product_line_id
    ...
    coe_jrr_cohort_active_customer
        start_month
        start_month_begin_date
        mrr
        block_count
...
Notes on how to filter when comparing to legacy data:
- Just grab ad counts and revenue and mrr.
- Try filtering on failed payment but actually think it is right not to.
- Legacy data had yearmonth screwed up when payment failed.  New code
  fixes that.  Impact is that in new code, some customers show up a
  little earlier than they did before.  Sometimes it has implications
  for their whole subsequent activity.
- Legacy code does un-proration and determining full price at
  customer level not subscription.
  Oh un-proration is at sub level, but the looking at last month
  and next month to triangulate happens at cust level.

LOOK the category NOT BILLED is a bit of a misnomer now.

Or maybe I have an additional category which is REVENUE ONLY?

tmp_data_dm.coe_jrr_backfill_customer_mrr_match_ss.
Ads only, and ignore expire date.
For that version, customer counts match ss exactly.
MRR differences basically come down to if a sub is discounted, the ss
does not know its full price until it is billed at full price.
