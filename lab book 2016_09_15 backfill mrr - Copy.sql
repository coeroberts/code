...
SELECT
  ...
  ,CASE WHEN new.customer_id IS NULL OR off.customer_id IS NULL THEN 'Customer mismatch' 
  ...
  ,CASE WHEN new.mrr_customer_category = off.mrr_customer_category THEN 'OK' ELSE 'Category mismatch' END AS chk_category
  ...
  ,new.has_ads_prior AS                        new_has_ads_prior
...
ORDER BY IFNULL(new.customer_id, off.customer_id), IFNULL(new.year_month, off.year_month)
