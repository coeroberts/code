...
- See if I can make it filter to full tenure quarters, because the
  ...
  I am just not going to show charts for absolute numbers.
  But LOOK I am not sure this totally solves it.
...
-- Acq Only / Ret Only / Acq + Ret
-- Wrap back to defn w/ Jake. Or not: Ret is return date cohort.

-- Retained in this context: anybody who was active in the month 
-- who did not churn in the month.
-- (In terms of MRR category, it''s ACQUIRED + RETAINED + PENETRATED + DOWNSIZED + NO ACTIVITY)

Churn: Dollars, customers, %
Net: 
one line per cohort
% defined as churned customers / prior month''s customers
both cust and mrr
X axis is tenure month
Y axis is %

For each chart, have a tab w/ the data behind it so they can repro charts.

More tables:
Churned MRR
Churned customer counts.

Maybe bar chart of what % of each month''s churn is each cohort.
X is calendar month.
Want this soon!

Month / Quarter
Acq Only / Return Only / Acq + Returned
Calendar Period / Tenure Period
Churned / Retained.
Customers / MRR
----

Think I need an Acquired / Returned field.
Nope that would change granularity and screw the averages up.
Actually it does not screw it up.  Done.

So here''s what''s weird.  To roll month up to quarter, I think I
want to sum across cohort months but average across tenure months?
Well it''s kinda sum everything and divide by the number of months
in the period (3 for quarter)?
If I do this right does it handle the is_complete_month flag too?
  First add up cohort months for the quarter.
  Then add up those across tenure months for the tenure quarter and div by 3.
  Order of ops is not crucial because same result.

OK when I look at 2013 year, there are only 108 months of data
available (as opposed to 144), because my data intentionally
does not start until April 2013.
Makes it even better to handle partially-missing data correctly.
(altho year rollup is not a hard requirement I suspect.)

I think what I want to do is sum all months, then divide by:
(data months present) / length of period.
It would be messier if I allowed different period lengths for
cohort and tenure, but currently I do not.

Still need to do period_complete_status right.
Not as easy as I''d hoped.  Kinda need to do some sort of cross join
thing to get potential cohort_x_tenure grids?

Churned mrr will not work the way I did it
because for customers it is the difference, but not for mrr.
we need the mrr that goes with those customers.
They cheated in the ss - it is not mrr from the churned customers.

select * from tmp_data_dm.coe_churn_by_cohort
where cust_type = 'Acquired'
and cohort_type = 'Quarter'
and cohort_year = 2016
and tenure_num <= 9
order by cohort, tenure_num
