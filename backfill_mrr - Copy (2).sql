...
SELECT
  ...
  ,(FLOOR(future.year_month/100)-FLOOR(nc.start_month/100)) * 12 +
         (future.year_month%100-       nc.start_month%100) AS tenure_month_lifetime
...
WHERE mth.is_active_during_month = 'N' AND mth.revenue <> 0
)
INSERT OVERWRITE TABLE tmp_data_dm.coe_jrr_cust_chains PARTITION(start_month_chain)
SELECT
   customer_id
...
SELECT
  ...
  ,CASE WHEN mrr_customer_category = 'NO ACTIVITY' THEN 'No Activity'  -- LOoK make one for revenue but never active during month
