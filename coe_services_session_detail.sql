DROP VIEW IF EXISTS tmp_data_dm.coe_services_session_detail;
CREATE VIEW tmp_data_dm.coe_services_session_detail AS
SELECT
   chk.*
  ,CONCAT(
     ...
     CASE WHEN chk_net_trust_account_deposit               LIKE 'E%' THEN CONCAT(chk_net_trust_account_deposit, ';  ') ELSE '' END,
  ...
  ,CONCAT(
     ...
     CASE WHEN chk_net_trust_account_deposit               LIKE 'W%' THEN CONCAT(chk_net_trust_account_deposit, ';  ') ELSE '' END,
  ...
  ,CONCAT(
     ...
     CASE WHEN chk_net_trust_account_deposit               LIKE 'N%' THEN CONCAT(chk_net_trust_account_deposit, ';  ') ELSE '' END,
...
SELECT
  ...
  ,CASE WHEN payable_after_refund_count > 0
         AND attorney_trust_account_withdraw_count = attorney_trust_account_deposit_succeeded_count 
             THEN 'Error: Payable after refund but no net trust account deposit'
             ELSE '' END AS chk_net_trust_account_deposit
  ...
  ,CASE WHEN refund_count > 0
        ...
        WHEN refund_count > 0
         AND transaction_amount_refund <> (transaction_amount_ataw - purchase_discount_amount)
...
FROM
  ...
  SELECT
     ...
     evt.advice_session_id_hacked AS       advice_session_id
    ...
    ,TO_DATE(MIN(evt.event_datetime)) AS         session_start_date
    ,        MIN(evt.event_datetime) AS          session_start_tm
    ,TO_DATE(MAX(evt.event_datetime)) AS         session_last_date
    ,        MAX(evt.event_datetime) AS          session_last_tm
    ,TO_DATE(MIN(CASE WHEN evt.event_category = 'cc_charge' THEN evt.event_datetime ELSE NULL END)) AS  cc_charge_date
    ,        MIN(CASE WHEN evt.event_category = 'cc_charge' THEN evt.event_datetime ELSE NULL END) AS   cc_charge_tm
    ,TO_DATE(MIN(CASE WHEN evt.event_category = 'attorney_trust_account_deposit_succeeded' THEN evt.event_datetime ELSE NULL END)) AS  first_trust_account_deposit_date
    ,MAX(evt.purchase_transaction_amount) AS  purchase_transaction_amount
    ,MAX(evt.purchase_discount_amount) AS     purchase_discount_amount
    ...
    ,MIN(1) AS sessions
  FROM prsn.services_event_detail evt
  GROUP BY 1
  ) ses
) chk
