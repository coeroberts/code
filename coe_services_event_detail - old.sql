...
DROP TABLE IF EXISTS tmp_data_dm.coe_services_event_detail_old;
CREATE TABLE tmp_data_dm.coe_services_event_detail_old AS
SELECT
  ...
  ,CASE
     ...
     ELSE 0 END AS     successful_purchase_price
  ,CASE
     WHEN evt.event_category = 'cc_charge' THEN evt.purch_discount_amount
     ELSE 0 END AS     successful_discount
  ,CASE
  ...
  ,CASE
     ...
     ELSE 0 END AS     net_transactions
  ,CASE
     WHEN evt.event_category = 'cc_charge' THEN      evt.purch_transaction_amount + evt.purch_discount_amount
...
FROM
  ...
  FROM
    ...
    SELECT
       evt.id AS                                       event_id
      ,evt.created_at_pst AS                           event_tm
      ,evt.event_type
